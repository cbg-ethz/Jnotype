"""Misspecified model (continuous latent traits)."""
import dataclasses
...
class Config:
    # Latent factors
    n_a: int
    dist_a: Literal["uniform", "gaussian"]
...
rule generate_data:
    ...
    run:
        ...
        # Generate (continuous)
        n_a = config.n_a
        if config.dist_a == "uniform":
            latent_traits = rng.uniform(0, 1, size=(config.n_samples, n_a))
        elif config.dist_a == "gaussian":
            latent_traits = rng.normal(0, 1, size=(config.n_samples, n_a))
        else:
            raise ValueError(f"Unknown distribution {config.dist_a}")
